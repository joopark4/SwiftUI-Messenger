...
public struct RoomArchiveEmptyItemSection: View {
    ...
    let pageType: RoomArchivePageType

    public init(contentsType: RoomArchiveContentsType = .FILE, pageType: RoomArchivePageType = .LOCAL) {
    ...
    private func getSubTitle2Message() -> String {
        if pageType == .LOCAL {
            ...
            switch contentsType {
            case .FILE:
                return tm.lang.localized("ChatRoom.File.No.AddInfo")
            case .LINK:
                return tm.lang.localized("ChatRoom.Link.No.AddInfo")
            case .ALBUM:
                return tm.lang.localized("ChatRoom.Album.No.AddInfo")
            }
        }
    }
}
